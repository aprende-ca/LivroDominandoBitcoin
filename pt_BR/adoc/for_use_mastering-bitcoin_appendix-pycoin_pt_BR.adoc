...
((("pycoin library")))A biblioteca Python https://github.com/richardkiss/pycoin[+pycoin+], originalmente escrita e mantida por Richard Kiss, é uma biblioteca baseada em Python que suporta a manipulação de chaves e transações bitcoin, suportando até mesmo a linguagem de script, o suficiente para lidar com transações fora do padrão.

A biblioteca pycoin suporta tanto o Python 2 (2.7.x) quanto o Python 3 (3.3 e posteirores), e vem com alguns utilitários de linha de comando úteis, o +ku+ e o +tx+.

=== Utilitário de Chave (Key Utility, KU)

((("key utility (ku)", id="keyutil17")))O utilitário de linha de comando +ku+ ("key utility, utilitário de chave") é um canivete suíço para a manipulação de chaves. Ele suporta chaves BIP-32, WIF e endereços (bitcoin e alt coins). A seguir, alguns exemplos.

Criar uma chave privada BIP-32 usando fontes de entropia padrão GPG e _/dev/random_:
...
Criar uma chave BIP-32 a partir de uma frase de senha (_passphrase_):

[WARNING]
====
A frase de senha usada nesse exemplo é fácil demais de ser descoberta.
====
...
Adquirir informações como JSON:
...
Verificar se isso funciona ao escolher uma _string_ BIP-32 (a que corresponde à subchave 0/3):
...
((("", startref="pycoin library")))Como um endereço Dogecoin:
...
O utilitário de linha de comando +tx+ irá exibir transações em formato legível por humanos, buscar transações básicas do cache de transações da pycoin ou de serviços da web (blockchain.info, blockcypher.com, blockr.io e chain.so são atualmente suportados), mesclar transações , adicionar ou remover entradas ou saídas e assinar transações.

A seguir temos alguns exemplos.


Veja a famosa transação "pizza":
...
uso: tx [-h] [-t TRANSACTION_VERSION] [-l LOCK_TIME] [-n NETWORK] [-a]
          ...
          argumento [argumento ...]
tx: erro: não foi possível encontrar o Tx com o id 49d2adb6e476fa46d8357babf78b1b501fd39e177ac7833124b3f67b17c40c2a
----


Opa! Não definimos os serviços web. Vamos fazer isso agora:
...
Isso não é feito automaticamente, portanto uma ferramenta de linha de comando não vazará para um website de terceiros informações potencialmente privadas sobre as transações que você está interessado. Se você não se importar com isso, você pode inserir essas linhas em seu _.profile_.
...
** não foi possível validar a transação, pois estão faltando as transações-fonte
----
...
todos os valores da transação de recebimento foram validados
----

((("", startref="TX17")))Agora, vamos ver as saídas não gastas (UTXOs) para um endereço específico. No bloco nº1, vemos uma transação coinbase para +12c6DSiU4Rq3P4ZxziKxzrL5LmMBrzjrJX+. Vamos usar +fetch_unspent+ para encontrar todas as moedas nesse endereço:
